...
EventList.prototype = {
   ...
   /**
    * Return the lambda id given the scheduledEventId
    * @param {String} scheduledEventId
    * @returns {String} lambdaId - The lambda id if found, false otherwise
    */
   lambdaIdFor: function (scheduledEventId) {
      ...
      for (i = 0; i < this._events.length; i++) {
         var evt = this._events[i];
         if (evt.eventId === scheduledEventId &&
             evt.lambdaFunctionScheduledEventAttributes
         ) {
  ...
  _has_event_with_attribute_value: function(attributeKey, attributeValue, eventType) {
   ...
   /**
    * This method returns true if the eventType already occured for the given activitylambda task Id
    * @param {String} activityId
    * @param {String} eventType
    * @returns {Boolean}
    */
    ...
    * @returns {Boolean}
    */
   _has_event_for_scheduledEventId: function(eventType, id) {
      var attrsKey = this._event_attributes_key(eventType);
      return this._events.some(function (evt) {
          if (evt.eventType === eventType) {
             if (this.activityIdFor(evt[attrsKey].scheduledEventId) === id ||
                 this.lambdaIdFor(evt[attrsKey].scheduledEventId) === id
             ) {
                return true;
             }
          }
   ...
   is_lambda_started: function (lambdaId) {
      return this._has_eventType_for_lambdaId(lambdaId, "LambdaFunctionStarted");
   },
   ...
   has_lambda_timedout: function (lambdaId) {
      return this._has_event_for_scheduledEventId('LambdaFunctionTimedOut', lambdaId);
   },
   ...
   has_lambda_failed: function (lambdaId) {
      return this._has_event_for_scheduledEventId('LambdaFunctionFailed', lambdaId);
   },

    /**
    * returns true if the scheduled lambda has failed
    * @param {String} lambdaId
    * @returns {Boolean}
    */
   has_schedule_lambda_failed: function (lambdaId) {
      return this._has_event_for_scheduledEventId('ScheduleLambdaFunctionFailed', lambdaId);
   },

   /**
    * returns true if start lambda has failed
    * @param {String} lambdaId
    * @returns {Boolean}
    */
   has_start_lambda_failed: function (lambdaId) {
      return this._has_event_for_scheduledEventId('StartLambdaFunctionFailed', lambdaId);
   },
    ...
    */
   failed: function (id) {
      return this.has_activity_failed(id) ||
             this.has_schedule_activity_task_failed(id) ||
             this.has_lambda_failed(id) ||
             this.has_schedule_lambda_failed(id) ||
             this.has_start_lambda_failed(id);
   },

   /**
    * Returns failed event
    * @param {String} id
    * @returns {Object} evt - The event if found, null otherwise
    */
   find_failed_event: function (id) {
      var failEventTypes = [
        'ActivityTaskFailed',
      ...
      return this._events.find(function(evt) {
        if (failEventTypes.indexOf(evt.eventType) < 0 ) {
            return false;
        }

        return this._has_event_for_scheduledEventId(evt.eventType, id);
      }, this);
   },

   /**
    * Returns true if the id timed out
    * @param {String} id
    * @returns {Boolean}
    */
   timed_out: function (id) {
      return this.has_activity_timedout(id)||
             this.has_lambda_timedout(id);
   },
   ...
   scheduled: function () {
      ...
      for (i = 0; i < arguments.length; i++) {
         if (!this.is_activity_scheduled(arguments[i]) &&
             !this.is_lambda_scheduled(arguments[i])
         ) {
            return false;
         }
      }
   ...
   /**
    * returns true if no Activity or lambda has been scheduled yet...
    * @returns {Boolean}
    */
   has_workflow_just_started: function () {
      ...
      for (i = 0; i < this._events.length; i++) {
         ...
         if (evtType === "ActivityTaskScheduled" ||
             evtType === "LambdaFunctionScheduled"
         ) {
            return false;
         }
      }
   ...
   has_lambda_completed: function (lambdaId) {
      return this._has_event_for_scheduledEventId('LambdaFunctionCompleted', lambdaId);
   },
   ...
   /**
    * Get the results for the given id
    * @param {String} id
    * @returns {Mixed}
    */
   results: function (id) {
      ...
      for (i = 0; i < this._events.length; i++) {
         ...
         if (evt.eventType === "ActivityTaskCompleted") {
            if (this.activityIdFor(evt.activityTaskCompletedEventAttributes.scheduledEventId) === id) {
         ...
         } else if (evt.eventType === "LambdaFunctionCompleted") {
            if (this.lambdaIdFor(evt.lambdaFunctionCompletedEventAttributes.scheduledEventId) === id) {
